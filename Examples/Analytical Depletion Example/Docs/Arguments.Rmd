...
<br/>
<br/>
<b><font size = "3">stream_depletion_output</font></b>: *char* default *'volumetric'*, describes whether the output of the program should be A) the volume of depletions experienced by each reach or B) depletions as a fraction of the pumping contributing to those depletions. Accepts *'volumetric' or 'fractional'*. **WARNING:** *'fractional'* works best under relatively constant pumping rates. Though the program attempts to correct for this, depletions occuring without pumping, due to travel time of pressure signal from well pumping, can easily cause a blow up to infinity.
<br/>
<br/>
<b><font size = "3">lagged_depletions_start_time</font></b>: *numeric* default *0.1*, if *stream_depletion_output* is set to *'fractional'* this describes where to start summing pumping for the denominator of depletions/pumping. As pumping does not immediately effect stream depletions, this is saying 'find the average time at which 1% of depletions occur for pumping on this reach'. Pumping is then summed between [:t-lagged_depletions_start_time]. Accepts any value between 0.01 and 0.99, however it should not be larger than *lagged_depletions_end_time*.Average time is calculated as a weighted average, where wells are weighted by proximity. This prevents a sudden increase in proximate well pumping to not enter the denominator until an inappropriate time, blowing up the fraction to infinity.
<br/>
<br/>
<b><font size = "3">lagged_depletions_end_time</font></b>: *numeric* default *0.90*, if *stream_depletion_output* is set to *'fractional'* this describes where to sum pumping for the denominator of depletions/pumping. As pumping does not effect stream depletions indefinitely, this is saying 'find the average time at which 99% of depletions occur for pumping on this reach'. Pumping is then summed between [t-lagged_depletions_end_time:]. Accepts any value between 0.01 and 0.99, however it should not be smaller than *lagged_depletions_start_time*. Average time is calculated as a weighted average, where wells are weighted by proximity. This prevents a sudden increase in proximate well pumping to not enter the denominator until an inappropriate time, blowing up the fraction to infinity.
<br/>
<br/>
